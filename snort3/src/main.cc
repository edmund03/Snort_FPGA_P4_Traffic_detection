...
/**
 * @brief temp function for debug
 *
 * @param info
 * @author yifei.zhou
 */
void temp_log_out(std::string info)
{
    const char *path = "/home/edmund/log/debug_log.txt";
    std::fstream file;
...
// fpga模式下间隔调用读取函数
bool fpga_quit_flag = false;
void fpga_read()
{
    while (!fpga_quit_flag)
    {
        // std::cout << "fpga read thread is running..." << std::endl;
        // temp_log_out("fpga read thread is running...");
        read_fpga_log();
...
// p4模式下间隔调用读取函数
bool p4_quit_flag = false;
void p4_read()
{
    while (!p4_quit_flag)
    {
        // std::cout << "p4 read thread is running..." << std::endl;
        // temp_log_out("p4 read thread is running...");
        read_p4_log();
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }
}

int distribution_path = -1;
int pre_distribution_path = -1;
int running_mode = 0;
...
// 添加循环获取环境变量的线程处理方法
void pattern_read() // 500ms读取一次pattern环境变量
{
    const char *cmd_source = "/bin/bash -c 'source /root/.bashrc && env | grep pattern'";
    char result[1024];
    char pattern[10];
    while (true)
    {
        ...
        executeCMD(cmd_source, result);
        strncpy(pattern, result + 8, sizeof(pattern));
        std::string pattern_string = pattern;
        pattern_string.erase(std::remove(pattern_string.begin(), pattern_string.end(), '\n'),
                             pattern_string.end());
        if (pattern_string == "ids")
        {
            distribution_path = 0;
        }
        else if (pattern_string == "ips")
        {
            distribution_path = 1;
        }
        else if (pattern_string == "p4")
        {
            distribution_path = 2;
        }
        else if (pattern_string == "fpga")
        {
            distribution_path = 3;
        }
        else
        {
            std::cout << "Distribution_path(IDS 0,IPS 1,P4 2,FPGA 3)?";
            std::cin >> distribution_path;
        }
        if (pre_distribution_path == distribution_path)
            continue;
        std::cout << "current pattern:" << pattern_string << std::endl;
        temp_log_out("current pattern:" + pattern_string);
        if ((pre_distribution_path == 1 && pattern_string != "ips") ||
            (pre_distribution_path == 2 && pattern_string != "p4"))
        {
            std::cout << "killing p4 read thread..." << std::endl;
            temp_log_out("killing p4 read thread...");
            // kill p4 thread
            p4_quit_flag = true;
            std::this_thread::sleep_for(std::chrono::milliseconds(200));
            // std::cout << "p4 end flag:" << p4_end_flag << std::endl;
            // del p4 rules
            del_p4_data_ipv4();
            del_p4_data_ipv6();
            p4_quit_flag = false;
            std::cout << "killed p4 read thread..." << std::endl;
            temp_log_out("killed p4 read thread...");
        }
        if (pre_distribution_path == 3 && pattern_string != "fpga")
        {
            std::cout << "killing fpga read thread..." << std::endl;
            temp_log_out("killing fpga read thread...");
            // kill fpga thread
            fpga_quit_flag = true;
            std::this_thread::sleep_for(std::chrono::milliseconds(200));
            // del fpga rules

            fpga_quit_flag = false;
            std::cout << "killed fpga read thread..." << std::endl;
            temp_log_out("killed fpga read thread...");
        }
        if (distribution_path == 1 || distribution_path == 2)
        {
            // 增加读取p4的线程
            std::thread p(p4_read);
            pthread_t thread_handle = p.native_handle();
            struct sched_param params;
            params.sched_priority = sched_get_priority_min(SCHED_FIFO);
            int res = pthread_setschedparam(thread_handle, SCHED_FIFO, &params);
            p.detach();
        }
        if (distribution_path == 3)
        {
            // 增加读取fpga的线程
            std::thread f(fpga_read);
            pthread_t thread_handle = f.native_handle();
            struct sched_param params;
            params.sched_priority = sched_get_priority_min(SCHED_FIFO);
            int res = pthread_setschedparam(thread_handle, SCHED_FIFO, &params);
            f.detach();
        }
        pre_distribution_path = distribution_path;
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < argc; ++i)
    {
        ...
        {
            printf("snort docker is running in host mode...\n");
            running_mode = 1;
        }
    }

    std::thread p(pattern_read);
    pthread_t thread_handle = p.native_handle();
    struct sched_param params;
    params.sched_priority = sched_get_priority_min(SCHED_FIFO);
    int res = pthread_setschedparam(thread_handle, SCHED_FIFO, &params);
